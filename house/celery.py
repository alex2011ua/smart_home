...
def setup_periodic_tasks_off_lights(sender, **kwargs):
    try:
        sender.add_periodic_task(
            crontab(minute=58, hour=20 - time_correct), lights_off.s(), name="lights_off"
        )
    except SoftTimeLimitExceeded as err:
        Logs.objects.create(
            date_log=datetime.datetime.now(),
...
def setup_periodic_tasks_pool_filtering_start(sender, **kwargs):
        sender.add_periodic_task(
            crontab(minute=30, hour=23-time_correct), start_filtering.s(), name="start_pool_filtering"
        )
...
def setup_periodic_tasks_pool_filtering_stop(sender, **kwargs):
        sender.add_periodic_task(
            crontab(minute=59, hour=23 - time_correct), stop_filtering.s(), name="stop_pool_filtering"
        )


#recirculation pool cleaner
@cellery_app.on_after_configure.connect
def setup_periodic_tasks_pool_cleaner(sender, **kwargs):
        sender.add_periodic_task(
